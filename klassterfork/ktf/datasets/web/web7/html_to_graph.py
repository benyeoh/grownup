...
def initialize_options(opt_parser):
    ...
    For 7-web-genre dataset, ideal output is .tfrecord per .html
    """
...
def get_input_files(opt_args):
    ...
    for class_dir in classes_dir:
        ...
        # Note: pay attention to the raw input file format. For 7-web each webpage is saved as .htm
        input_files[class_label] = [path for path in glob.glob(os.path.join(
            opt_args.input_dir, class_dir, "*.htm")) if os.path.isfile(path)]
