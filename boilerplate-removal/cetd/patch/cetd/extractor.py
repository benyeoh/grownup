...
def mark_content(web_element: Tag, threshold: float):
    ...
    if mark != 1 and text_density >= threshold:  # in paper algorithm 2 is text_density >= threshold, orignal src too
...
def find_max_density_sum_tag(web_element: Tag, max_density_sum: float):
    ...
    if mark != 1:
        set_mark(target, 1)
        # if target.name.lower() != "body": # Ada use another part
        #     parent = target.parent
        #     while True:
        #         parent[KG_MARK] = 2  # magic constant
        #         if parent.name.lower() == "body":
...
class AbstractExtractor(ABC):
    ...
    def create_doc(self, html: str) -> Tag:
        ...
        # Re-parse the document with lxml after initial parsing by the default parser
        # print(str(doc.prettify()))
        doc = BeautifulSoup(str(doc), "lxml")
        return doc.body
    ...
    def extract_content(self, html: str) -> str:
       ...
       # logger.debug("Density sums: ")
        ...
        print("Threshold is {}".format(threshold)) # Ada print
        ...
        if web_element.next_element is not None:
            ...
            for child in web_element.children:
                if isinstance(child, Tag):
                    ...
                    tag_num += child[KG_TAG_NUM] + 1 # auto increment of tag number, following C++ src
...
class Extractor(AbstractExtractor):
    ...
    # def compute_density_sum(self, web_element: Tag, ratio: float):
    #     """
    #     Recursively computes the sum of text density across the node provided and all subnodes
    #     :param web_element:
    #     :param ratio:
    #     :return:
    #     """
    ...
    #                         # if length > 0 and ratio > 0: # ada test original condition with bug fix
    #                             density_sum += length * ln(1. * length) / ln(ln(ratio * length + math.e))
    #                         from_ = index + len(child_content) # ada move one layer up
    ...
    # Use the CTD formula (L400) instead of the simplified version in C++ src
    def compute_density_sum(self, web_element: Tag, ratio: float):
        """
        Recursively computes the sum of text density across the node provided and all subnodes
        :param web_element:
                ...
                for child in web_element.children:
                    if isinstance(child, Tag):
                        ...
                        if index > -1:
                            length = index - from_
                            if length > 0 and ratio * length > (1 / math.e):
                                density_sum += (char_num / tag_num) * ln(
                                    (char_num * tag_num) / (linkchar_num * linktag_num)) / ln(
                                    ln(char_num * linkchar_num / un_linkchar_num + ratio * char_num + math.e))
                            from_ = index + len(child_content) # from_ is updated regardless of length value, follow the logic of C++ src
...
class VariantExtractor(AbstractExtractor):
    ...
    def count_chars(self, web_element: Tag, force_zero=False):
        """
        ...
        if isinstance(web_element, Tag):
            if not is_ignorable(web_element, self.ignorable_tags) and not force_zero:
                # print("Here: %s" % web_element)
                for child in web_element.children:
                    if isinstance(child, Tag):
                        self.count_chars(child)
                for child in web_element.children:
                ...
                for child in web_element.children:
                    if isinstance(child, Tag):
                        self.count_chars(child, force_zero=True)
...
if __name__ == '__main__':
    sample = open('/Users/blevine/composite-text-density/SEC-AAPL-10K.txt',
