...
class ConvGraphNet(SuperModel):
    ...
    def __init__(self,
        ...
        Args:
            ...
            max_feature_size: (Optional) The maximum cap for the size of hidden features.
            ...
            expansion_rate: (Optional) The expansion rate of the number of hidden units for every new layer. Default is 1.0
        ...
        """
        super(ConvGraphNet, self).__init__(**kwargs)
        ...
        for i, l in enumerate(layers):
            output_size = min(int((i * expansion_rate + 1) * hidden_base_feature_size), max_feature_size)
            block = [ConvGraphNetBlock(output_size,
            ...
            for j in range(1, l):
                block.append(ConvGraphNetBlock(output_size,
                                               self_loops_only=True,
                                               normalization=normalization,
                                               use_residuals=use_residuals,
                                               dropout=dropout,
    ...
    def call(self, inputs, training=False):
        ...
        for block in self._prelim_blocks:
            features = block((adjacency, features), training)

        for block in self._blocks:
            for conv_graph_blk in block:
                features = conv_graph_blk((adjacency, features), training)

        if self._feature_block:
            features = self._feature_block((adjacency, features), training)
